defmodule SahajyogWeb.UserLive.Confirmation do
      ...
      <div class="min-h-screen flex items-center justify-center bg-base-200 px-4 py-12">
        <div class="w-full max-w-md">
          <div class="text-center mb-8">
            <h1 class="text-4xl font-bold mb-3">{gettext("Welcome %{email}", email: @user.email)}</h1>
          </div>
  ...
  def mount(%{"token" => token}, _session, socket) do
    if user = Accounts.get_user_by_magic_link_token(token) do
      ...
      {:ok,
       assign(socket,
         page_title: gettext("Confirm Account"),
         user: user,
         form: form,
